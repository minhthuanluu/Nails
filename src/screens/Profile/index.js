...
export default class ProfileScreen extends Component {
  ...
  updateImage = async image => {
    ...
    formData.append(
      'avatarUser',
      Platform.OS === 'android' ? image.replace('file://', '') : image,
    );
    try {
      let response = await axios({
        method: 'POST',
        url: 'https://nail2go-server.herokuapp.com/api/users/editAvatar',
        data: formData,
        headers: {
  ...
  async componentDidMount() {
    ...
    var langeChoosed = langList.filter(item => item.id === language);
    this.setState({token, langeChoosed});
      ...
      return (
        <View
          ...
          {this.state.errorResult.length > 0 ? (
            ...
            <Text style={{color: '#000', fontFamily: fonts.regular}}>
              <AppText i18nKey="not_empty_noti"/>
            </Text>
          )}
        </View>
      );
          ...
          <View style={{flexGrow: 1}}>
            ...
            <View style={profile.PointInfo}>
              <View style={profile.point}>
                ...
                <Text style={profile.Infotitle}><AppText i18nKey="point" /> </Text>
              </View>
              <View style={profile.booked}>
                <Text style={profile.content}>{this.state.data.point}</Text>
                <Text style={profile.Infotitle}><AppText i18nKey="booked" /> </Text>
              </View>
            </View>
            <View style={profile.subFunction}>
              <Text style={profile.functionTitle}><AppText i18nKey="settings" /></Text>
              <View style={profile.functionTools}>
                <Text
                  ...
                  <AppText i18nKey="language" /> 
                </Text>
                <TouchableOpacity
                  onPress={() =>
                    this.props.navigation.navigate('ChangeLanguage',{profile: 'profile'})
                  }>
                  <Text
                    style={{
                      fontFamily: fonts.regular,
                    ...
                    {this.state.langeChoosed
                      ? <AppText i18nKey="lang"/>
                      //this.state.langeChoosed[0].label
                      : <AppText i18nKey="lang"/>
                      }
                  </Text>
                </TouchableOpacity>
            ...
            <TouchableOpacity
              ...
              <Text style={profile.logginText}><AppText i18nKey="logout" /></Text>
            </TouchableOpacity>
          </View>
        </ScrollView>
      </SafeAreaView>
